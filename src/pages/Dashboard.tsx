...
export default function Dashboard() {
  ...
  const avgDoshaBalance = {
    vata: Math.round(
      patients.reduce((acc, p) => acc + p.doshaBalance.vata, 0) /
        patients.length
    ),
    pitta: Math.round(
      patients.reduce((acc, p) => acc + p.doshaBalance.pitta, 0) /
        patients.length
    ),
    kapha: Math.round(
      patients.reduce((acc, p) => acc + p.doshaBalance.kapha, 0) /
        patients.length
    ),
  };
            ...
            <div className="p-4 bg-subtle-gradient rounded-lg border border-border">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  <Badge className="bg-vata text-white text-xs">
                    V:{avgDoshaBalance.vata}%
                  </Badge>
                  <Badge className="bg-pitta text-white text-xs">
                    P:{avgDoshaBalance.pitta}%
                  </Badge>
                  <Badge className="bg-kapha text-white text-xs">
                    K:{avgDoshaBalance.kapha}%
                  </Badge>
                </div>
              </div>
              <p className="text-xs text-muted-foreground">
